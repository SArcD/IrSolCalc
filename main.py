...
with tab2:
    ...
    def generate_radiation_data(latitude, day_of_year):
        """Genera los datos de radiación UV y total para cada hora del día."""
        hours_of_day = np.arange(0, 24, 0.5)  # Horas del día en intervalos de 0.5 horas
        radiations = []
        uv_radiations = []
        altitudes = []
    ...
    # Configuración de Streamlit
    st.title("Variación de Radiación Solar")
    st.write("Explora cómo varía la radiación solar (total y UV) a lo largo del día según la latitud y el día del año.")

    # Inputs del usuario en la barra lateral
    #st.sidebar.header("Parámetros de Entrada")
    #latitude = st.sidebar.slider("Latitud (°)", -90.0, 90.0, 19.43, step=0.1)
    #day_of_year = st.sidebar.slider("Día del Año", 1, 365, 172)
    radiation_type = st.sidebar.radio("Selecciona el tipo de radiación:", ["Radiación Total", "Radiación UV"])

    # Generar datos
    df = generate_radiation_data(latitude, day_of_year)

    # Elegir tipo de radiación
    if radiation_type == "Radiación Total":
        y_axis = "Radiación Total (W/m²)"
        title = f"Variación de Radiación Total para Latitud {latitude}° - Día del Año {day_of_year}"
    else:
        y_axis = "Radiación UV (W/m²)"
    ...
    fig = px.line(
        ...
        y=y_axis,
        title=title,
        labels={"Hora del Día": "Hora del Día", y_axis: y_axis},
    )
    fig.update_layout(
        xaxis_title="Hora del Día",
        yaxis_title=y_axis,
